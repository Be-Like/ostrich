...
typedef struct {
    ...
    long build_pgid;      /* parsed from PID marker (for ABORT)   */
    int build_exit;       /* parsed from __OSTRICH_EXIT__ marker  */
    bool have_build_exit; /* true once the EXIT marker is seen    */
    /* Partial-line filter for __OSTRICH_* marker suppression (build step). */
    char build_line_buf[256];
    int build_line_len; /* >=0: buffering; -1: passthrough mode */
...
static void emit_ev(WorkerCtx *ctx, ConnPhase phase, SshStatus reason, bool hk_unknown, bool hk_mismatch) {
    SessionEvent ev;
    ...
    if (ctx->cfg.user[0] || ctx->cfg.host[0]) {
        snprintf(ev.user_host, sizeof(ev.user_host), "%s@%s", ctx->cfg.user, ctx->cfg.host);
    }
    if (hk_unknown || hk_mismatch) memcpy(ev.fingerprint, ctx->fingerprint, sizeof(ev.fingerprint));
    spsc_push(ctx->s->event_ring, &ev);

    LOG_INFO(LG_CONN, "phase → %s%s%s reason=%s", connstate_phase_str(phase), hk_unknown ? " [unknown-hostkey]" : "",
             hk_mismatch ? " [hostkey-mismatch]" : "", ssh_status_str(reason));
}
...
static void emit_run_phase(WorkerCtx *ctx, RunPhase phase, BdStatus reason) {
    ...
    ev.reason = reason;
    push_run_ev(ctx, &ev);
    LOG_INFO(LG_RUN, "phase → %s reason=%s", runstate_phase_str(phase), bd_status_str(reason));
}
...
static void emit_stale(WorkerCtx *ctx, bool stale) {
    ...
    ev.stale = stale;
    push_run_ev(ctx, &ev);
    LOG_INFO(LG_RUN, "stale=%s built_gen=%d deployed_gen=%d", stale ? "true" : "false", ctx->rs.built_gen,
             ctx->rs.deployed_gen);
}
...
static void start_run_chain(WorkerCtx *ctx, const SessionRunCmd *cmd, bool compile_only) {
    ...
    rc->step = (ctx->kc_pass[0] != '\0') ? RCHAIN_STEP_KC_UNLOCK : RCHAIN_STEP_SETTINGS;
...
static void start_run_abort(WorkerCtx *ctx, bool is_reexecute, const SessionRunCmd *pending) {
    ...
    if (rc->active && rc->build_pgid > 0) {
        if (bd_kill_cmd(rc->build_pgid, ra->kill_cmd, sizeof(ra->kill_cmd)) == BD_OK) ra->need_kill = true;
    }
    ...
    if (dc->state != DCON_IDLE) {
        ...
        if (bd_terminate_cmd(&tgt, dc->bundle_id, ra->term_cmd, sizeof(ra->term_cmd)) == BD_OK) ra->need_term = true;
    }
    ...
    LOG_INFO(LG_RUN, "abort: start need_term=%d need_kill=%d is_reexecute=%d", ra->need_term, ra->need_kill,
             is_reexecute);
}
...
static void fail_run_chain(WorkerCtx *ctx, RunEvent ev, BdStatus reason) {
    ...
    LOG_WARN(LG_RUN, "chain fail step=%s reason=%s", rchain_step_str(rc->step), bd_status_str(reason));
}
...
static void handle_step_exit(WorkerCtx *ctx, int exit_code) {
    ...
    switch (rc->step) {
        case RCHAIN_STEP_KC_UNLOCK: {
            if (exit_code != 0) {
                char block[2048];
                if (bd_unlock_help_block(ctx->cfg.user, ctx->cfg.host, ctx->cfg.port, block, sizeof(block)) == BD_OK) {
        ...
        case RCHAIN_STEP_BUILD: {
            /* Flush any partial line left in the marker-filter buffer. */
            if (rc->build_line_len > 0) {
                static const char PFX[] = "__OSTRICH_";
                bool marker = (rc->build_line_len >= 10 && memcmp(rc->build_line_buf, PFX, 10) == 0);
                if (marker) {
                    rc->build_line_buf[rc->build_line_len] = '\0';
                    Str s = {rc->build_line_buf, (size_t)rc->build_line_len};
                    int code = 0;
                    if (bd_parse_exit_marker(s, &code)) {
                        rc->build_exit = code;
                        rc->have_build_exit = true;
                    }
                } else {
                    emit_build_log(ctx, rc->build_line_buf, (size_t)rc->build_line_len);
                }
                rc->build_line_len = 0;
            }

            /* Use the in-band EXIT marker when available; fall back to the channel
               exit code (the setsid-missing case where no marker was ever emitted). */
            int eff_exit = rc->have_build_exit ? rc->build_exit : exit_code;

            if (eff_exit != 0 && rc->build_pgid == 0) {
                char block[2048];
                if (bd_setsid_help_block(ctx->cfg.user, ctx->cfg.host, ctx->cfg.port, block, sizeof(block)) == BD_OK) {
                    emit_build_log(ctx, block, strlen(block));
                }
            ...
            if (eff_exit != 0) {
                if (!rc->target.is_simulator && ctx->kc_pass[0] == '\0') {
                    char hint[1024];
                    if (bd_codesign_hint_block(ctx->cfg.user, ctx->cfg.host, ctx->cfg.port, hint, sizeof(hint)) ==
                        BD_OK) {
                        emit_build_log(ctx, hint, strlen(hint));
                    }
                }
                BdStatus r = (eff_exit == 127) ? BD_ERR_XCODE_MISSING : BD_ERR_BUILD;
                fail_run_chain(ctx, RUN_EV_BUILD_FAIL, r);
                return;
            }
            ...
            if (act == RUN_ACT_DONE) {
                ...
                rc->active = false;
                if (runstate_stale(&ctx->rs)) emit_stale(ctx, true);
                return;
            }
            emit_run_phase(ctx, ctx->rs.phase, BD_OK);
...
/*
 * Process a chunk from the build step, filtering __OSTRICH_* control tokens.
 * Marker lines (those beginning with "__OSTRICH_") are silently consumed and
 * fed to the PGID / EXIT parsers; all other lines are forwarded to emit_build_log.
 * State is kept in rc->build_line_buf / rc->build_line_len across calls so
 * that a marker split across two read-chunks is handled correctly.
 * build_line_len >= 0: buffering into the line buffer.
 * build_line_len == -1: passthrough mode (line definitely not a marker).
 */
static void process_build_chunk_filtered(WorkerCtx *ctx, const char *buf, size_t n) {
    static const char PFX[] = "__OSTRICH_";
    static const int PFX_LEN = 10; /* sizeof("__OSTRICH_") - 1 */
    ...
    while (pos < n) {
        if (rc->build_line_len < 0) {
            /* Passthrough: emit bytes up to and including the next newline. */
            size_t j = pos;
            while (j < n && buf[j] != '\n') j++;
            if (j < n) {
                j++;
                rc->build_line_len = 0;
            } /* newline found; exit passthrough */
            if (j > pos) emit_build_log(ctx, buf + pos, j - pos);
            pos = j;
        } else if (rc->build_line_len < (int)(sizeof(rc->build_line_buf) - 1)) {
            /* Buffering: append byte to line buffer. */
            char c = buf[pos++];
            rc->build_line_buf[rc->build_line_len++] = c;
            if (c == '\n') {
                bool marker = (rc->build_line_len >= PFX_LEN && memcmp(rc->build_line_buf, PFX, (size_t)PFX_LEN) == 0);
                if (marker) {
                    rc->build_line_buf[rc->build_line_len] = '\0';
                    Str s = {rc->build_line_buf, (size_t)rc->build_line_len};
                    long pgid = 0;
                    if (bd_parse_pid_marker(s, &pgid) && pgid > 0) {
                        rc->build_pgid = pgid;
                    ...
                    if (bd_parse_exit_marker(s, &code)) {
                        rc->build_exit = code;
                        rc->have_build_exit = true;
                        LOG_INFO(LG_RUN, "build exit marker=%d", code);
                    }
                } else {
                    emit_build_log(ctx, rc->build_line_buf, (size_t)rc->build_line_len);
                }
            ...
            /* Buffer full without newline — cannot be a marker; emit and enter passthrough. */
            emit_build_log(ctx, rc->build_line_buf, (size_t)rc->build_line_len);
            rc->build_line_len = -1;
        }
    }
...
static void process_run_chunk(WorkerCtx *ctx, const char *buf, size_t n) {
    ...
    switch (rc->step) {
        ...
        case RCHAIN_STEP_SETTINGS:
            ...
            break;
        case RCHAIN_STEP_BUILD:
            process_build_chunk_filtered(ctx, buf, n);
            break;
        case RCHAIN_STEP_PRIME:
        case RCHAIN_STEP_INSTALL:
            emit_build_log(ctx, buf, n);
        ...
        switch (rc->state) {
            ...
            case RCHAIN_EXEC: {
                ...
                if (rc->step == RCHAIN_STEP_LAUNCH) {
                    ...
                    if (blen >= sizeof(ctx->dev_console.bundle_id)) blen = sizeof(ctx->dev_console.bundle_id) - 1;
                    memcpy(ctx->dev_console.bundle_id, rc->cfg.bundle_id, blen);
                    ctx->dev_console.bundle_id[blen] = '\0';
                    size_t ulen = strlen(rc->target.udid);
                    if (ulen >= sizeof(ctx->dev_console.udid)) ulen = sizeof(ctx->dev_console.udid) - 1;
...
static void drain_run_cmds(WorkerCtx *ctx) {
    ...
    while (spsc_pop(ctx->s->run_cmd_ring, &cmd)) {
        switch (cmd.kind) {
            case RCMD_EXECUTE:
                handle_run_execute(ctx, &cmd);
                break;
            case RCMD_COMPILE:
                handle_run_compile(ctx, &cmd);
                break;
            case RCMD_ABORT:
                handle_run_abort(ctx);
                break;
            case RCMD_SET_KC_PASS:
                handle_set_kc_pass(ctx, &cmd);
                break;
        }
    }
}
...
static SessionDiscEventKind failure_event_for(DiscJobKind kind) {
    switch (kind) {
        case DJOB_KIND_READ_BLUEPRINT:
            return DEV_BLUEPRINT_FAILED;
        case DJOB_KIND_RESOLVE_BUNDLE_ID:
            return DEV_BUNDLE_ID_FAILED;
        case DJOB_KIND_SWEEP_DEVICECTL:
        case DJOB_KIND_SWEEP_SIMCTL:
            return DEV_SWEEP_FAILED;
        default:
            return DEV_SCAN_FAILED;
    }
}
...
static void fail_disc_job(WorkerCtx *ctx, int i, DiscStatus st) {
    ...
    bool is_sweep = (j->kind == DJOB_KIND_SWEEP_DEVICECTL || j->kind == DJOB_KIND_SWEEP_SIMCTL);
    if (is_sweep) {
...
static const char *djob_kind_str(DiscJobKind k) {
    switch (k) {
        ...
        case DJOB_KIND_RESOLVE_BUNDLE_ID:
            return "resolve-bundle-id";
        case DJOB_KIND_SWEEP_DEVICECTL:
            return "sweep-devicectl";
        case DJOB_KIND_SWEEP_SIMCTL:
            return "sweep-simctl";
        default:
            return "none";
        ...
        switch (j->state) {
...
#endif
                LOG_INFO(LG_DISC, "exec job=%d kind=%s cmd=\"%s\"", i, djob_kind_str(j->kind), j->cmd);
                j->state = DJOB_READ;
                again = true;
                break;
            }
            ...
            case DJOB_EXIT: {
...
#ifdef OSTRICH_DEBUG
                {
                    struct timespec now = mono_now();
                    long ms =
                        (now.tv_sec - j->exec_start.tv_sec) * 1000L + (now.tv_nsec - j->exec_start.tv_nsec) / 1000000L;
                    LOG_INFO(LG_DISC, "done job=%d exit=%d ms=%ldms bytes=%zu", i, exit_code, ms, j->buf_len);
                    LOG_BLOB(LOG_DEBUG, LG_DISC, "output", j->buf, j->buf_len);
                }
#endif
                if (exit_code == 127) {
                    fail_disc_job(ctx, i, DISC_ERR_XCODE_MISSING);
                    return;
                }
                if (exit_code != 0) {
                    fail_disc_job(ctx, i, DISC_ERR_COMMAND_FAILED);
                ...
                }
                {
                    ...
                    switch (j->kind) {
                        case DJOB_KIND_SCAN:
                            ds = disc_curate_blueprints(ctx->disc_arenas[i], raw, j->scan_depth, &j->curated);
                            break;
                        case DJOB_KIND_READ_BLUEPRINT:
                            ds = disc_parse_list(ctx->disc_arenas[i], raw, &j->schemes, &j->configs);
                            break;
                        case DJOB_KIND_RESOLVE_BUNDLE_ID:
                    ...
                    if (ds != DISC_OK) {
                        LOG_WARN(LG_DISC, "parse-fail job=%d kind=%s status=%s", i, djob_kind_str(j->kind),
                                 disc_status_str(ds));
...
static void handle_disc_scan(WorkerCtx *ctx, const SessionDiscCmd *cmd) {
    ...
    int slot = -1;
    for (int i = 0; i < DISC_MAX_JOBS; i++) {
        if (ctx->disc_jobs[i].kind == DJOB_KIND_NONE) {
            slot = i;
            break;
        }
    }
...
static void handle_disc_resolve_bundle_id(WorkerCtx *ctx, const SessionDiscCmd *cmd) {
    ...
    DiscStatus ds = disc_build_settings_cmd(cmd->project, cmd->scheme, cmd->config, j->cmd, sizeof(j->cmd));
    if (ds != DISC_OK) {
        SessionDiscEvent ev;
        memset(&ev, 0, sizeof(ev));
...
static void handle_disc_sweep(WorkerCtx *ctx) {
    ...
    for (int i = 0; i < DISC_MAX_JOBS; i++) {
        ...
        if (k == DJOB_KIND_SWEEP_DEVICECTL || k == DJOB_KIND_SWEEP_SIMCTL) return; /* sweep in flight — silently drop */
        if (k == DJOB_KIND_NONE && found < 2) slots[found++] = i;
    }
    if (found < 2) {
        SessionDiscEvent ev;
        memset(&ev, 0, sizeof(ev));
...
static void drain_disc_cmds(WorkerCtx *ctx) {
    ...
    while (spsc_pop(ctx->s->disc_cmd_ring, &cmd)) {
        switch (cmd.kind) {
            case DCMD_SCAN_HOST:
                handle_disc_scan(ctx, &cmd);
                break;
            case DCMD_ABORT_SCAN:
                handle_disc_abort(ctx);
                break;
            case DCMD_READ_BLUEPRINT:
                handle_disc_read_blueprint(ctx, &cmd);
                break;
            case DCMD_RESOLVE_BUNDLE_ID:
                handle_disc_resolve_bundle_id(ctx, &cmd);
                break;
            case DCMD_SWEEP_TARGETS:
                handle_disc_sweep(ctx);
                break;
        }
    }
}
...
static void handle_breach(WorkerCtx *ctx, const SshConfig *cfg) {
    ...
    ctx->cfg = *cfg;
    LOG_INFO(LG_CONN, "breach: connecting to %s@%s:%d", cfg->user, cfg->host, cfg->port);

    SshStatus st = ssh_connect_start(ctx->arena, ctx->cfg, &ctx->ssh, &ctx->ssh_fd);
    if (st != SSH_OK) {
    ...
    while (spsc_pop(ctx->s->cmd_ring, &cmd)) {
        switch (cmd.kind) {
            case CMD_BREACH:
                ...
                break;
            case CMD_ABORT:
                if (ctx->sub != SUB_IDLE && ctx->sub != SUB_ONLINE) handle_abort_close(ctx, EV_ABORT);
                break;
            case CMD_CLOSE:
                if (ctx->sub != SUB_IDLE) handle_abort_close(ctx, EV_CLOSE);
                break;
            case CMD_TRUST:
                handle_trust(ctx);
                break;
    ...
    switch (ctx->sub) {
        ...
        case SUB_BACKOFF:
            if (ctx->has_deadline && deadline_past(&ctx->deadline)) {
                ...
                st = ssh_connect_start(ctx->arena, ctx->cfg, &ctx->ssh, &ctx->ssh_fd);
                if (st != SSH_OK) {
                    on_fail(ctx, EV_FAIL, st);
                    return;
                }
        ...
        case SUB_HANDSHAKE:
            ...
            }
            {
                ...
                st = ssh_hostkey_check(ctx->ssh, &verdict, ctx->fingerprint, sizeof(ctx->fingerprint));
                if (st != SSH_OK) {
                    on_fail(ctx, EV_FAIL, st);
                    return;
                }
...
static int compute_timeout_ms(const WorkerCtx *ctx) {
    switch (ctx->sub) {
        ...
        case SUB_ONLINE:
            ...
            if (has_active_disc_jobs(ctx) || ctx->run_chain.active || ctx->dev_console.state != DCON_IDLE ||
