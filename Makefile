...
ifeq ($(UNAME_S),Linux)
# GLFW backends compiled into the binary. Default builds both X11 and Wayland;
# GLFW picks one at runtime (XDG_SESSION_TYPE / WAYLAND_DISPLAY / DISPLAY).
# Narrow it for a leaner build env, e.g. `make GLFW_BACKENDS=x11`.
GLFW_BACKENDS ?= x11 wayland

# GLFW 3.4 dlopen()s the X11/Wayland/GL client libraries at runtime, so none of
# them belong on the link line. -lGL stays only because our own UI code
# (src/ui/ui.cpp) calls glClear/glClearColor/glViewport directly; libGL
# (libglvnd) is present with any Mesa install and needs no X server. Not linking
# the X11 client libs keeps the binary loadable on a pure-Wayland host.
GLFW_DEFS     :=
PLATFORM_LIBS := $(LOCAL_LFLAGS) -ldl -lpthread -lm -lGL
GLFW_PLAT_SRC := \
    posix_time.c posix_thread.c posix_module.c posix_poll.c \
    xkb_unicode.c linux_joystick.c

ifneq (,$(findstring x11,$(GLFW_BACKENDS)))
GLFW_DEFS     += -D_GLFW_X11
GLFW_PLAT_SRC += x11_init.c x11_monitor.c x11_window.c glx_context.c
endif

ifneq (,$(findstring wayland,$(GLFW_BACKENDS)))
GLFW_DEFS     += -D_GLFW_WAYLAND
GLFW_PLAT_SRC += wl_init.c wl_monitor.c wl_window.c
# The Wayland sources need protocol glue generated by wayland-scanner from the
# XML vendored in GLFW's deps/, plus the wayland-client + xkbcommon *headers*
# (the libs themselves are dlopen()ed at runtime, not linked).
WLGEN          := $(BUILD)/glfw/wl-protocols
WAYLAND_CFLAGS := $(shell pkg-config --cflags wayland-client xkbcommon 2>/dev/null)
GLFW_GEN_INC   := -I$(WLGEN) $(WAYLAND_CFLAGS)
WL_PROTOCOLS   := wayland viewporter xdg-shell xdg-activation-v1 \
    xdg-decoration-unstable-v1 idle-inhibit-unstable-v1 \
    pointer-constraints-unstable-v1 relative-pointer-unstable-v1 \
    fractional-scale-v1
WL_PROTO_HEADERS := \
    $(patsubst %,$(WLGEN)/%-client-protocol.h,$(WL_PROTOCOLS)) \
    $(patsubst %,$(WLGEN)/%-client-protocol-code.h,$(WL_PROTOCOLS))
endif
...
# ── GLFW ──────────────────────────────────────────────────────────────
# GLFW_GEN_INC is empty unless the Wayland backend is enabled (Linux only).
$(BUILD)/glfw/%.o: $(GLFW_DIR)/src/%.c | $(BUILD)/glfw
	$(CC) -c $(GLFW_DEFS) $(GLFW_GEN_INC) -I$(GLFW_DIR)/include -I$(GLFW_DIR)/src \
	    $(LOCAL_CFLAGS) -w -O2 -o $@ $<
...
# ── Wayland protocol codegen (only when the Wayland backend is selected) ──
ifneq (,$(findstring wayland,$(GLFW_BACKENDS)))
$(WLGEN)/%-client-protocol.h: $(GLFW_DIR)/deps/wayland/%.xml | $(WLGEN)
	wayland-scanner client-header $< $@

$(WLGEN)/%-client-protocol-code.h: $(GLFW_DIR)/deps/wayland/%.xml | $(WLGEN)
	wayland-scanner private-code $< $@

$(WLGEN):
	mkdir -p $(WLGEN)

# The Wayland sources #include the generated protocol headers above.
$(BUILD)/glfw/wl_init.o $(BUILD)/glfw/wl_window.o $(BUILD)/glfw/wl_monitor.o: \
    $(WL_PROTO_HEADERS)
endif
